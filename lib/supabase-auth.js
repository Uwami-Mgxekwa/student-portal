...
if (!supabase) {
  console.error('❌ Supabase client is not initialized. Check your internet connection and browser settings.');
}
...
export async function getSchedule(course, certificate, year = null, showLoadingOverlay = true) {
  ...
  try {
    ...
    // Get general schedule (year = 'all' or year = 1 as fallback)
    const { data, error } = await supabase
      .from('schedules')
      .select('*')
      .eq('course', course)
      .eq('certificate', certificate)
      .or('year.eq.all,year.eq.1')
      .maybeSingle();
