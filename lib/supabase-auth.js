  ...
  try {
    ...
    const { error: studentInfoError } = await supabase
      ...
      .insert({
        ...
        faculty: getFacultyForCourse(studentData.course),
...
export async function getSchedule(course, certificate, year = null) {
  ...
  try {
    ...
    if (year) {
      const { data: yearData, error: yearError } = await query
        .eq('year', year)
        .maybeSingle();
      ...
      if (!yearError && yearData) {
        ...
        removeLoading();
        return { success: true, schedule: yearData };
      }
      
      // If no year-specific schedule, try 'all' years
      logInfo('Year-specific schedule not found', 'Trying general schedule');
    }
    
    // Get general schedule (year = 'all')
